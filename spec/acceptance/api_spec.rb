...
feature "API Feature" do
  ...
  scenario "organizations show" do
    visit "/api/v1/organizations/1.json?token=#{@al.authentication_token}"
    json = JSON.parse page.body
    
    json["organization"]["id"].should == 1
  ...
  scenario "projects index" do
    visit "/api/v1/organizations/1/projects.json?token=#{@al.authentication_token}"
    json = JSON.parse page.body
    
    json.count.should == 2
    
    json.first["project"]["id"].should == 1
    ...
    json.second["project"]["id"].should == 2
  ...
  scenario "projects index, filter by user IDs" do
    visit "/api/v1/organizations/1/projects.json?token=#{@al.authentication_token}&users=1"
    json = JSON.parse page.body
  ...
  scenario "projects show" do
    visit "/api/v1/organizations/1/projects/1.json?token=#{@al.authentication_token}"
    json = JSON.parse page.body
    
    json["project"]["id"].should == 1
  ...
  scenario "users index" do
    visit "/api/v1/organizations/1/users.json?token=#{@al.authentication_token}"
    json = JSON.parse page.body
    json.count.should == 1
    json.first["user"]["id"].should == 1
